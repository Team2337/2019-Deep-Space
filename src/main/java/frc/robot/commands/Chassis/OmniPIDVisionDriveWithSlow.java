 ...
 * @author Bryce G.
 */
public class OmniPIDVisionDriveWithSlow extends PIDCommand {

  double turnValue, targetAngle, leftJoystick, m_speed, speed, m_timeout, ta, tx, speedModifier, smallP, largeP;
  double p, i, d;
   ...
   */
  public OmniPIDVisionDriveWithSlow(double smallP, double largeP) {
    super("PIDLimelightTurn", smallP, 0, 0);        // set name, P, I, D.
    ...
    getPIDController().setOutputRange(-0.24, 0.24);       // output range for 'turn' input to drive command
  ...
  protected void usePIDOutput(double output) {
    ...
    if(Math.abs(tx) < 8) {
      this.getPIDController().setPID(smallP, 0, 0); 
    } else {
      this.getPIDController().setPID(largeP, 0, 0);
    }
